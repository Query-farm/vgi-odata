...
#
# The odata worker queries OData services over HTTP, so the suite needs a
# server: this script builds the repo's `mockserver`, starts it on a free port,
# and points the tests at it via VGI_ODATA_TEST_URL (mirroring `make test-sql`).
#
# Required environment:
#   HAYBARN_UNITTEST   path to the haybarn-unittest binary
#   VGI_ODATA_WORKER   worker LOCATION the .test files ATTACH (the built Go
...
# --- Start the mock OData server (the .test files query it) -----------------
...
# VGI_ODATA_TEST_URL, and kill the server on exit — exactly like `make test-sql`.
...
echo "Running suite (worker: $VGI_ODATA_WORKER) ..."
